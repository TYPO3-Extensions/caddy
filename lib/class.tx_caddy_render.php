 ...
 * @since       1.4.6
 */
class tx_caddy_render extends tslib_pibase {
	public $prefixId = 'tx_caddy_pi1';
	public $scriptRelPath = 'pi1/class.tx_caddy_pi1.php';
	public $extKey = 'caddy';
	
	
	public function renderOptionById($conf, $type, $option_id, &$obj) {
		...
		if (array_key_exists($optionIds['extra'], $filterArr)) {
			foreach ($optionIds['extra.'] as $extra) {
				if (floatval($extra['value']) <= $filterArr[$optionIds['extra']]) {
					$option = $this->renderOption($conf, $type, $option_id, floatval($extra['extra']), $obj);
				} else {
					if ($option == '') {
						return $conf[$type.'.']['options.'][$option_id . '.']['title'] . ' (0,00 ' . $conf['main.']['currencySymbol'] . ') <br />';
					}
					break;
				}
			}
			...
			switch ($optionIds['extra']) {
				case 'each':
					...
					$option = $this->renderOption($conf, $type, $option_id, $gross, $obj);
					break;
				default:
	...
	public function renderOption($conf, $type, $option_id, $gross, $obj) {
		$free_from = $conf[$type.'.']['options.'][$option_id . '.']['free_from'];
		$free_until = $conf[$type.'.']['options.'][$option_id . '.']['free_until'];
		if ((isset($free_from) && (floatval($free_from) <= $obj->cartGrossNoService)) || (isset($free_until) && (floatval($free_until) >= $obj->cartGrossNoService))) {
			return $conf[$type.'.']['options.'][$option_id . '.']['title'] . ' (0,00 ' . $conf['main.']['currencySymbol'] . ') <br />';
		}

	 	return $conf[$type.'.']['options.'][$option_id . '.']['title'] . ' (' . number_format($gross, 2, ',', '.') . ' ' . $conf['main.']['currencySymbol'] . ') <br />';
	}
}